...
export default function AgentsPage() {
  ...
  const strengthColor = (s: number) => s >= 4 ? 'text-green-600 dark:text-green-400' : s >= 2 ? 'text-yellow-600 dark:text-yellow-400' : 'text-text-muted'
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-text-primary">Agent 管理</h1>
          <span className="text-xs text-text-muted bg-bg-tertiary px-2 py-0.5 rounded">{agents.length} 个</span>
          {agents.some(a => a.is_default) && (
            <span className="text-xs text-amber-600 dark:text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded flex items-center gap-1">
        ...
        <button onClick={() => { resetForm(); setShowForm(true) }}
          disabled={submitting}
          className="flex items-center gap-2 bg-accent hover:bg-accent-hover disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm transition-colors">
      ...
      {showForm && (
        <div className="mx-6 mt-4 bg-bg-secondary border border-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-text-primary font-medium">{editingId ? '编辑 Agent' : '新建 Agent'}</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                className="w-full bg-bg-tertiary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:border-accent outline-none" placeholder="Agent 名称" />
            </div>
            <div>
              <label className="block text-xs text-text-muted mb-1">分类</label>
              <input value={form.category} onChange={e => setForm({...form, category: e.target.value})}
                className="w-full bg-bg-tertiary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:border-accent outline-none" placeholder="general" />
            </div>
            <div className="col-span-2">
              <label className="block text-xs text-text-muted mb-1">描述</label>
              <input value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                className="w-full bg-bg-tertiary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:border-accent outline-none" placeholder="简单描述 Agent 的用途（AI 将据此分析与推荐）" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs text-text-muted mb-1">配置模式</label>
            ...
            {form.setup_mode === 'quick' && (
              <div className="col-span-2">
                <div className="flex items-start gap-2 text-xs text-accent bg-accent/10 border border-accent/20 rounded-lg px-3 py-2 mb-2">
            ...
            {form.setup_mode === 'detailed' || form.showPrompt ? (
              <div className="col-span-2">
                <label className="block text-xs text-text-muted mb-1">
                  System Prompt {form.setup_mode === 'quick' && <span className="text-accent">（AI 草稿，可微调）</span>}
                </label>
            ...
            {/* 技能选择：从已安装技能搜索添加 */}
            <div className="col-span-2">
              <label className="block text-xs text-text-muted mb-1">技能（从已安装技能中搜索添加）</label>
              <div className="relative">
                <div className="flex items-center gap-2 bg-bg-tertiary border border-border rounded-lg px-3 py-2">
                  ...
                  {skillOpen && (
                    <button onClick={() => setSkillOpen(false)} className="text-text-muted hover:text-text-primary"><X size={14} /></button>
                  )}
                </div>
                {skillOpen && (
                  <div className="absolute z-20 mt-1 w-full max-h-48 overflow-auto bg-bg-secondary border border-border rounded-lg shadow-xl">
                    {filteredSkills.length === 0 ? (
                      ...
                      <div key={s.id} onClick={() => { toggleSkill(s.name); setSkillSearch('') }}
                        className="px-3 py-2 text-sm text-text-primary hover:bg-bg-tertiary cursor-pointer flex items-center gap-2">
                        <Code size={12} className="text-green-600 dark:text-green-400" /> {s.name}
                        <span className="text-[10px] text-text-muted truncate">{s.description}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {form.skills.map(s => (
                    <span key={s} className="text-[11px] px-2 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400 flex items-center gap-1">
                      <Code size={10} />{s}
                      <button onClick={() => toggleSkill(s)} className="hover:text-red-500 dark:hover:text-red-400"><X size={10} /></button>
                    </span>
                  ))}
                </div>
              )}
            ...
            {/* 标签 */}
            <div className="col-span-2">
              <label className="block text-xs text-text-muted mb-1">标签（AI 推荐或手动，逗号分隔）</label>
              <input value={form.tags.join(', ')} onChange={e => setForm({...form, tags: e.target.value.split(',').map(s => s.trim()).filter(Boolean)})}
                className="w-full bg-bg-tertiary border border-border rounded-lg px-3 py-2 text-text-primary text-sm focus:border-accent outline-none" placeholder="coding, assistant" />
            </div>
          </div>
          ...
          <div className="flex gap-6 mt-4 flex-wrap">
            {(['enable_planning', 'enable_rag', 'enable_reflection'] as const).map(key => (
              <label key={key} className="flex items-center gap-2 cursor-pointer text-sm text-text-muted">
          ...
          <div className="flex gap-6 mt-4 flex-wrap items-center">
            <div className="flex items-center gap-2 text-sm text-text-muted">
              ...
              <select value={form.config_mode} onChange={e => setForm({...form, config_mode: e.target.value as any})}
                className="bg-bg-tertiary border border-border rounded-lg px-2 py-1 text-text-primary text-sm focus:border-accent outline-none">
                <option value="global">全局默认配置</option>
                <option value="self">使用本 Agent 自带配置（读取自己的知识库等）</option>
              </select>
            </div>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-amber-600 dark:text-amber-300">
              <input type="checkbox" checked={form.is_default} onChange={e => setForm({...form, is_default: e.target.checked})}
                className="rounded bg-bg-tertiary border-border text-amber-500 focus:ring-amber-500" />
          ...
          <div className="flex gap-3 mt-5">
            ...
            <button onClick={resetForm} disabled={submitting}
              className="text-text-muted hover:text-text-primary px-4 py-2 text-sm transition-colors disabled:opacity-50">取消</button>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {agents.length === 0 ? (
          ...
          <div className="grid gap-3">
            {agents.map(a => (
              <div key={a.id} className="bg-bg-secondary border border-border rounded-xl p-4 hover:border-accent/40 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-text-primary font-medium truncate">{a.name}</span>
                      {a.is_default && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-600 dark:text-amber-300 flex items-center gap-1">
                          <Star size={9} /> 默认</span>
                      )}
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${a.status === 'active' ? 'bg-green-500/20 text-green-600 dark:text-green-400' : a.status === 'running' ? 'bg-blue-500/20 text-blue-600 dark:text-blue-400' : a.status === 'error' ? 'bg-red-500/20 text-red-600 dark:text-red-400' : 'bg-bg-tertiary text-text-muted'}`}>
                        {a.status === 'active' ? 'Active' : a.status === 'running' ? '运行中' : a.status === 'error' ? '异常' : 'Idle'}
                      </span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/20 text-accent">{modeLabel(a.setup_mode)}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-muted flex items-center gap-1">
                        {a.config_mode === 'global' ? <Globe size={9} /> : <UserIcon size={9} />}
                        {a.config_mode === 'global' ? '全局' : '自带'}
                      </span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-muted">{a.model}</span>
                    </div>
                    <p className="text-text-muted text-sm truncate">{a.description || '无描述'}</p>
                    {a.system_prompt && (
                      <p className="text-text-dim text-xs mt-1 truncate italic">"{a.system_prompt.slice(0, 100)}"</p>
                    )}
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      {a.tags.map((t, i) => (
                        <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-muted flex items-center gap-1">
                          <Tag size={10} />{t}
                        </span>
                      ))}
                      {a.skills.slice(0, 3).map((s, i) => (
                        <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400 flex items-center gap-1">
                          <Code size={10} />{s}
                        </span>
                      ))}
                      {a.skills.length > 3 && (
                        <span className="text-[10px] text-text-muted">+{a.skills.length - 3}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 ml-3">
                    <button onClick={() => startEdit(a)} className="p-1.5 text-text-muted hover:text-accent hover:bg-bg-tertiary rounded-lg transition-colors">
                      <Edit3 size={15} />
                    </button>
                    <button onClick={() => handleDelete(a.id)} className="p-1.5 text-text-muted hover:text-red-500 dark:hover:text-red-400 hover:bg-bg-tertiary rounded-lg transition-colors">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </div>
