...
## 产出类任务工作流（PPT/文档/图片/报告等）

生成型任务必须遵循「脚本 → 执行 → 验证」闭环，严禁只写脚本不执行：

1. **write_file** — 将完整的生成脚本写入 .py 文件（如 generate_ppt.py）
2. **run_terminal** — 执行该脚本：`python3 generate_ppt.py`（而非用 python3 -c 内联大段代码）
3. **确认产出** — 执行后用 `ls -la` 或 `list_files` 检查目标文件（.pptx/.docx/.png 等）是否已生成，告知用户结果

**禁止**：写完脚本就认为任务完成。没有执行就没有产出。
